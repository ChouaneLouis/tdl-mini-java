...
public abstract class AbstractField<RecordKind extends Expression> implements Expression {
	...
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
        boolean _result = this.record.completeResolve(_scope);
        ...
        RecordType _struct = (RecordType) _type;
        _result &= _struct.completeResolve(_scope);

        if (_struct.contains(this.name)) {
            this.field = _struct.get(this.name);
        } else {
            Logger.error("You're trying to access field " + this.name + "which does not exist.");
            _result = false;
        }
